...
parser.add_argument('--style_lambda', type=float, default=100, help='lambda for style loss')
...
def main():
    ...
    VGG2 = models.vgg19(pretrained=True).features
    ...
    for epoch in range(args.train_epoch):
        ...
        for (x, _), (y, _) in zip(train_loader_src, train_loader_tgt):
            ...
            # Calc Style Loss
            real_img_features = get_features(y, VGG2)
            fake_img_features = get_features(G_, VGG2)
            ...
            for layer in style_weights:
                r_img_feature = real_img_features[layer]
                f_img_feature = fake_img_features[layer]
                r_gram = gram_matrix(r_img_feature)
                f_gram = gram_matrix(f_img_feature)
                layer_style_loss = style_weights[layer] * torch.mean((r_gram - f_gram) **2)
                _, d, h, w = r_img_feature.shape
                Style_loss += layer_style_loss / (d * h * w)

            Style_loss = args.style_lambda * Style_loss 
        ...
        print(
        '[%d/%d] - time: %.2f, Disc loss: %.3f, Gen loss: %.3f, Con loss: %.3f, Style loss: %.3f' % ((epoch + 1), args.train_epoch, per_epoch_time, torch.mean(torch.FloatTensor(Disc_losses)),
            torch.mean(torch.FloatTensor(Gen_losses)), torch.mean(torch.FloatTensor(Con_losses)), torch.mean(torch.FloatTensor(Style_losses))))
